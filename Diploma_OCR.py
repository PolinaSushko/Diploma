...
CONTOUR_H_MARGIN_ERROR = 0.15
CONTOUR_W_MARGIN_ERROR = 0.15

# Load the model
model = load_model(MODEL_PATH)

# Load image
image = cv2.imread(EX_IM_PATH_8)
...
def show_prepr_im(im, blur_im, edge_im, morph_im):
    """
    ...
    images = [im, blur_im, edge_im, morph_im]
...
def preprocess_img(img, visualize = False):
    """
    ...
    2. Applies adaptive thresholding to enhance contrast, making edges more distinct.
    ...
    #adaptive_thresh = cv2.adaptiveThreshold(edged, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 15, 10)
    #adaptive_thresh = cv2.adaptiveThreshold(edged, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 15, 10)
    ...
    if visualize:
        show_prepr_im(img, blurred, edged, morph)
...
def get_median_min_max_dimensions(contours):
    """
...
def split_right_wrong_contours(contours, median_width, median_height, width_margin, height_margin):
    """
...
def merge_overlapping_boxes(contours):
    """
    ...
    for box in contours:
        ...
        for i, merged_box in enumerate(merged_contours):
            if do_boxes_overlap(box, merged_box):
                # Combine the boxes
                merged_contours[i] = combine_boxes(merged_box, box)
                overlap_found = True
                break
...
def check_box_alignment(box1, box2, gap_threshold = 25, overlap_ratio = 0.5):
    """
...
def process_wrong_contours(wrong_contours_list, median_width, median_height, width_margin, height_margin):
    """
    ...
    for i, (x_1, y_1, w_1, h_1) in enumerate(wrong_contours_list):
            ...
            for j, (x_2, y_2, w_2, h_2) in enumerate(wrong_contours_list):
                if (x_1 != x_2 or y_1 != y_2) and (i, j) not in combined_pairs and (j, i) not in combined_pairs and i not in used_indices and j not in used_indices:
                    ...
                    box_alignment = check_box_alignment((x_1, y_1, w_1, h_1), (x_2, y_2, w_2, h_2))
                    ...
                    if combined_w != None and combined_h != None:
                        ...
                        if (median_width - width_margin <= combined_w <= median_width + width_margin) and \
                            ...
                            start_x, start_y, combined_w, combined_h = combine_boxes((x_1, y_1, w_1, h_1), (x_2, y_2, w_2, h_2))
...
def split_bounding_boxes(line, wrong_contours_list, median_height, height_margin, width_to_height_ratio = 2.0, ratio_margin = 0.2):
    """
...
def group_bounding_boxes_into_lines(bounding_boxes, wrong_contours, height_margin, y_threshold = 10):
    """
...
def fill_gaps_in_lines(lines, wrong_contours_lines, median_width):
    """
...
def add_missing_bounding_boxes(lines, wrong_contours, median_width, median_height, width_margin, height_margin):
    """
    ...
    for line in lines:
        merged_contours = merge_overlapping_boxes(line)

        merged_contours_in_lines.append(merged_contours)

    for line in merged_contours_in_lines:
        combined_contours, not_combined = process_wrong_contours(line, median_width, median_height, width_margin, height_margin)
    ...
    for line in all_contours_in_line:
        line, new_wrong_contours = split_bounding_boxes(line, wrong_contours, median_height, height_margin)
    ...
    _, _, min_width, min_height, _, _ = get_median_min_max_dimensions(wrong_contours_without_splitted)
    ...
    result = fill_gaps_in_lines(updated_lines, wrong_contours_lines, median_width)
...
def detect_contours(original_img, edged_img):
    ...
    - result : A list of bounding box dimensions (x, y, w, h) for valid contours.
    """
    ...
    lines = group_bounding_boxes_into_lines(right_contours, wrong_contours, h_margin)

    if len(wrong_contours) > 0:
        result = add_missing_bounding_boxes(lines, wrong_contours, median_w, median_h, w_margin, h_margin)
    else:
        result = lines
    ...
    detected_image = draw_lines_on_image(detected_image, result)
    
    return detected_image, result

def recognize_characters(detected_img, resulted_cnts, model, character_dict):
    """
    ...
    - detected_img: The original image with green rectangles drawn around the detected and filtered contours.
    - result : A list of bounding box dimensions (x, y, w, h) for valid contours.
    - model: The trained machine learning model used to predict characters from the ROIs.
    - character_dict: A dictionary mapping the predicted model index to the corresponding character.
...
detected_img, resulted_contours = detect_contours(image, morph)

# Recognize characters with model and dictionary
recognized_img = recognize_characters(detected_img, resulted_contours, model, character_dict)
